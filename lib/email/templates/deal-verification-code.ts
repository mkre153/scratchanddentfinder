/**
 * Deal Verification Code Email Template
 *
 * Sent when a retailer submits a deal and needs to verify their email.
 */

interface DealVerificationCodeEmailProps {
  code: string
  dealTitle: string
}

export function DealVerificationCodeEmail({ code, dealTitle }: DealVerificationCodeEmailProps): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;">
  ...
  <div style="padding: 32px 24px; text-align: center;">
    <h2 style="color: #111827; font-size: 24px; font-weight: bold; margin-bottom: 16px;">Verify your deal posting</h2>

    <p style="color: #374151; font-size: 16px; line-height: 24px; margin-bottom: 24px;">
      You're posting a deal: <strong>${dealTitle}</strong>.<br>
    ...
    <p style="color: #9ca3af; font-size: 12px; line-height: 20px;">
      If you didn't submit this deal, you can safely ignore this email.
    </p>
  </div>
